    ...
    return this.each(function(i, e) {
      ...
      $this
        .bind(triggerEvent + '.simpledialog', function (event) {
          ...
          } else if (!!_t.opts.target) {
            _target = (typeof(_t.opts.target) == 'string') ? jQuery('#' + _t.opts.target) : _t.opts.target;
            _escapedContent = _target.clone().html();
            _target.empty();
            _show(_escapedContent);
          } else {
      ...
      if (!!opts.justnow && jQuery.support.noCloneEvent) {
        $this.triggerHandler(triggerEvent + '.simpledialog');
      }
    });
  };
